...
# Create .env file (never overwrite an existing one with fresh secrets —
# that would break a running deployment. Keep existing unless explicitly told to reset.)
if [ -f .env ] && [ "$FORCE_ENV_RESET" != "true" ]; then
    echo -e "${YELLOW}⚠️  An existing .env file was found — keeping it to avoid regenerating secrets.${NC}"
    echo -e "   To regenerate, delete .env or re-run with ${CYAN}FORCE_ENV_RESET=true${NC}."
else
    if [ -f .env ]; then
        echo -e "${YELLOW}⚠️  FORCE_ENV_RESET=true — overwriting existing .env.${NC}"
    fi
...
EOF
    ...
    fi # end: create .env (preserve existing unless FORCE_ENV_RESET)

echo -e "${GREEN}✅ Environment configuration created${NC}"

# Stop any running services (no -v: preserve database volumes — re-running setup
# must never wipe existing data. Use `docker compose down -v` manually for a clean slate.)
